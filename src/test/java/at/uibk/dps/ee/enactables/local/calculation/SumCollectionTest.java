...
public class SumCollectionTest {
	...
	protected static class SumCollectionMock extends SumCollection{
		public SumCollectionMock(Set<EnactableStateListener> stateListeners, Set<String> inputKeys, Task functionNode, JsonObject jsonInput) {
	...
	public void test() {
		...
		try {
			tested.atomicPlay();
			assertTrue(tested.getJsonResult().has(ConstantsCalculation.outputSumCollection));
			int result = tested.getJsonResult().get(ConstantsCalculation.outputSumCollection).getAsInt();
