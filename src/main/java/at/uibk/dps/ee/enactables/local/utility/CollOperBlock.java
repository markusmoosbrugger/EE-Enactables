...
/**
 * Utility function to transform a collection following the BLOCK operators as defined in the AFCL
 * language.
 * 
...
public class CollOperBlock implements CollOper {
   ...
   * 
   * @param subCollectionString the string defining the parameters of the block operation
   * @param jsonInput the JsonInput of the enactable (possibly storing the dynamic parameters)
   */
  ...
  public JsonElement transformCollection(final JsonArray originalCollection) {
    if (size == 1 && overlap == 0) {
      ...
      int entryIdx = 0;
      for (int start = 0; start - overlap + size < originalCollection.size(); start +=
          size - overlap) {
  ...
  /**
   * Returns the number of entries that the application of the block operator will result in.
   * 
   * @param originalSize
   * @param blockSize
   * @param overlap
